...
const PedigreeTree: React.FC<PedigreeTreeProps> = ({ pet, onPetClick, maxDepth = 3, className }) => {
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: WheelEvent) => {
            ...
            // Determine if it's a trackpad pinch (ctrlKey) or mouse wheel
            // User requested "scrolling to zoom", so we default wheel to zoom.

            const zoomIntensity = 0.002;
            const delta = -e.deltaY;
            const currentScale = transformRef.current.scale;
            let newScale = currentScale + (delta * zoomIntensity * currentScale); // Logarithmic-ish
    ...
    return (
        <div className={cn(
            ...
            className // Apply passed classNames (e.g. h-full)
        )}>
